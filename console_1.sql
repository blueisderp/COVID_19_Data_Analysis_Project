...
CREATE VIEW PercentPopulationInfectedByCountry AS
    Select Location, continent, Population, MAX(total_cases) as HighestInfectionCount,  Max(CAST(total_cases AS float)/population)*100 as PercentPopulationInfected
    From CovidDeaths
    --Where location like '%states%'
